...
def getDataofWeights(year_value, df1, df2, df3, df4):
    ...
    selected_data1 = df1[(df1['Period'] == year_value)][selected_columns1].copy()
    selected_data2 = df2[(df2['Period'] == year_value)][selected_columns1].copy()
    selected_data3 = df3[(df3['Period'] == year_value)][selected_columns1].copy()
    selected_data4 = df4[(df4['Period'] == year_value)][selected_columns1].copy()
...
def culDistCountryMatrix(carbon_cost, pivot_data, battery_scrap_num, battery_recyc_num, df_cost_fit_data_recycl, time,
    ...
    supply = battery_scrap_num['scrap_sum'].values
...
def OUTPUTNetProfitofOptimalOne(ev_battery_data_selected, pivot_diff, Recycling_revenue_df, carbon_cost,
    ...
    net_profit_strat1_chiose = net_profit_strat1.loc[idx, ['country', 'type', 'recycling_m', 'scrap_sum', 'net_profit']]
...
def culWeightAddScrap(routes_1, prod_scrap_type_chiose):
    ...
    routes_original_df = routes_original_df.rename(columns={'weights': 'scrap_sum'}).iloc[:, -3:]
    routes_original_df = routes_original_df.groupby(['iso3', 'type'])['scrap_sum'].sum().reset_index()

...
for year_val in val_list:
    ...
    #         columns={'Region': 'country', 'Undisposed_scrap': 'scrap'})
    #     no_recyc_cap_group_st['scrap'] = (no_recyc_cap_group_st['scrap'] * 1e4).apply(np.floor).astype(int)
    ...
    #     battery_selected_num = battery_selected_num[['iso3', 'scrap']].copy()
    #     battery_selected_num.index = battery_selected_num['iso3']
    #     battery_selected_num['scrap'] = battery_selected_num['scrap'].apply(np.floor).astype(int)
    #     battery_selected_num = battery_selected_num.drop('iso3', axis=1)
    #     battery_selected_num = battery_selected_num[battery_selected_num['scrap'] != 0]
    #
    #     # Demand
    ...
    #
    #     # 策略2
    #     exist_recyc_cap_com = exist_recyc_cap_com.rename(columns={'Region': 'country', 'Mass_t': 'scrap'})
    #     exist_recyc_cap_com = exist_recyc_cap_com.merge(nation_df_new, on='country', how='left')
    #     exist_recyc_cap_com = (exist_recyc_cap_com[['iso3', 'Mass_v', 'scrap']]).copy()
    #
    ...
    #     battery_selected_num4 = battery_selected_num4[['iso3', 'scrap']].copy()
    #     battery_selected_num4.index = battery_selected_num4['iso3']
    #     battery_selected_num4['scrap'] = battery_selected_num4['scrap'].apply(np.floor).astype(int)
    #     battery_selected_num4 = battery_selected_num4.drop('iso3', axis=1)
    #
    #     chois_buch = exist_recyc_cap_com[exist_recyc_cap_com['scrap'] != 0].copy()
    #
    #     chois_buch['scrap'] = (chois_buch['scrap'] * 1e4).astype(int)
    #     battery_selected_buch = chois_buch.iloc[:, :2]
    #     battery_selected_buch = battery_selected_buch[['scrap']].copy()
    #
    ...
    #
    #     battery_selected_num4 = battery_selected_num4[battery_selected_num4['scrap'] != 0]
    #
...
for recycl_proce in recycl_list:
    ...
    for year_val in val_list:
        ...
        for Strategy_type in ['Strategy 2', 'Strategy 3']:
            ...
            disposal_mass_diff_stra_df = disposal_mass_diff_stra_df.append(
                merged_scrap_df_merge_new[['country', 'type', 'recycling_m', 'scrap']])
            ...
            merged_scrap_recycling_emission = disposal_mass_diff_stra_df.merge(recycling_emission_df, on=['type', 'recycling_m'], how='left')
            ...
            net_profit_strat1_chiose2 = add_scenario_columns(net_profit_strat1_chiose2)
            net_profit_strat = add_scenario_columns(net_profit_strat)
            net_profit_strat1_chiose2.to_csv(result_path(year_val, f'net_profit_{recycl_proce}_{Strategy_type}.csv'), index=True)

            net_profit_strat['total_netprofits'] = net_profit_strat['scrap'] * net_profit_strat['net_profit'] * 1000
            ...
            net_profit_strat_country.to_csv(result_path(year_val, f'country_net_profit_{recycl_proce}_{Strategy_type}.csv'), index=True)

            net_profit_strat['total_netprofits'] = net_profit_strat['scrap'] * net_profit_strat[
                'net_profit'] * 1000
            ...
            net_profit_strat['total_costs'] = net_profit_strat['scrap'] * net_profit_strat['unit_cost'] * 1000
            ...
            cost_strat_country.to_csv(result_path(year_val, f'country_cost_{recycl_proce}_{Strategy_type}.csv'), index=True)
        ...
        disposal_mass_diff_stra_df = disposal_mass_diff_stra_df.append(
            ev_battery_type_selected[['country', 'type', 'scrap']])
        disposal_mass_diff_stra_df['recycling_m'] = [recycl_proce] * len(disposal_mass_diff_stra_df)
        disposal_mass_diff_stra_df['year'] = [year_val] * len(disposal_mass_diff_stra_df)
        ...
        net_profit_strat1.to_csv(result_path(year_val, f'net_profit_all_{recycl_proce}_Strategy 1.csv'), index=True)
        net_profit_strat1_chiose1.to_csv(result_path(year_val, f'net_profit_{recycl_proce}_Strategy 1.csv'), index=True)

        net_profit_strat1['total_netprofits'] = net_profit_strat1['scrap'] * net_profit_strat1['net_profit'] * 1000
        net_profit_strat_country = net_profit_strat1.groupby('country')[
            ['total_netprofits', 'scrap']].sum().reset_index()
        net_profit_strat_country['net_profit'] = net_profit_strat_country['total_netprofits'] / \
                                                 net_profit_strat_country['scrap'] / 1000

        net_profit_strat1['total_costs'] = net_profit_strat1['scrap'] * net_profit_strat1['unit_cost'] * 1000
        cost_strat_country = net_profit_strat1.groupby('country')[['total_costs', 'scrap']].sum().reset_index()
        cost_strat_country['unit_cost'] = cost_strat_country['total_costs'] / cost_strat_country['scrap'] / 1000

        cost_strat_country = add_scenario_columns(cost_strat_country)
        net_profit_strat_country = add_scenario_columns(net_profit_strat_country)
        cost_strat_country.to_csv(result_path(year_val, f'country_cost_{recycl_proce}_Strategy 1.csv'), index=True)
        net_profit_strat_country.to_csv(result_path(year_val, f'country_net_profit_{recycl_proce}_Strategy 1.csv'), index=True)
        disposal_mass_diff_stra_df1 = disposal_mass_diff_stra_df1.append(disposal_mass_diff_stra_df)
        CO2_em_disposal_mass_diff_stra = CO2_em_disposal_mass_diff_stra.append(merged_scrap_recycling_emission)
    ...
    disposal_mass_diff_stra_df1.to_csv(result_path(f'country_disposal_mass_diff_stra_{recycl_proce}.csv'), index=True)
    CO2_em_df.to_csv(result_path(f'CO2_em_trans_{recycl_proce}.csv'), index=True)
    CO2_em_disposal_mass_diff_stra.to_csv(result_path(f'CO2_em_disposal_mass_diff_stra_{recycl_proce}.csv'), index=True)

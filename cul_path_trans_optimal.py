...
recycling_rate_scenario = os.environ.get('RECYCLING_RATE_SCENARIO')
recovery_efficiency_scenario = os.environ.get('RECOVERY_EFFICIENCY_SCENARIO')
scenario_run = bool(recycling_rate_scenario or recovery_efficiency_scenario)
collection_scenario_name = recycling_rate_scenario or 'original_collection'
recovery_scenario_name = recovery_efficiency_scenario or 'existing_recovery'
if scenario_run:
    trans_paths_root = f'./trans/scenario_paths/{collection_scenario_name}/{recovery_scenario_name}'
    trans_result_root = f'./trans/scenario_result/{collection_scenario_name}/{recovery_scenario_name}'
else:
    trans_paths_root = './trans/paths'
    trans_result_root = './trans/result'


def scenario_path(year, filename):
    path = f'{trans_paths_root}/{year}/{filename}'
    fallback_path = f'./trans/paths/{year}/{filename}'
    if scenario_run and not os.path.exists(path):
        return fallback_path
    return path


def result_path(*parts):
    return os.path.join(trans_result_root, *[str(part) for part in parts])


def add_scenario_columns(df):
    if scenario_run and df is not None and not df.empty:
        df = df.copy()
        df['collection_scenario'] = collection_scenario_name
        df['recovery_efficiency_scenario'] = recovery_scenario_name
    return df


def load_li_recovery_lookup():
    if not recovery_efficiency_scenario:
        return pd.DataFrame()
    recovery_file = './technology_lithium_recovery_scenarios.csv'
    if not os.path.exists(recovery_file):
        return pd.DataFrame()
    recovery_df = pd.read_csv(recovery_file)
    recovery_df = recovery_df[
        recovery_df['recovery_efficiency_scenario'] == recovery_efficiency_scenario
    ].copy()
    return recovery_df[['Year', 'recycling_m', 'li_recovery_efficiency']]


def add_lithium_metrics(df, li_content_dict, li_recovery_lookup):
    ...
    df = df.drop(columns=[col for col in metric_cols if col in df.columns])
    df['li_content'] = df['type'].map(li_content_dict).fillna(0)
    df['contained_lithium'] = df['scrap'] * df['li_content']
    if not li_recovery_lookup.empty:
        df = df.merge(
            li_recovery_lookup,
        ...
        df['li_recovery_efficiency'] = 0
    df['li_recovery_efficiency'] = df['li_recovery_efficiency'].fillna(0)
    df['recycled_lithium'] = df['contained_lithium'] * df['li_recovery_efficiency']
    df['primary_lithium_offset'] = df['recycled_lithium']
    df['primary_lithium_gap'] = df['contained_lithium'] - df['recycled_lithium']
    return df


if recycling_rate_scenario:
    scenario_file = './Scenario result/recycling_rate/EV_battery_inuse_scrap_collected_by_scenario.csv'
    scenario_data = pd.read_csv(scenario_file)
    scenario_data = scenario_data[scenario_data['scenario'] == recycling_rate_scenario].copy()
    if scenario_data.empty:
        raise ValueError(f'No recycling-rate scenario found: {recycling_rate_scenario}')
    evb_type_data = scenario_data.copy()
    evb_data = evb_type_data.groupby(['Year', 'region'])[['scrap', 'inuse']].sum().reset_index()
    evb_data = evb_data.rename(columns={'inuse': 'inuse_sum'})
    print(f'Using recycling-rate scenario: {recycling_rate_scenario}')
else:
    evb_type_data = evb_type_data.copy()
evb_data_batt = evb_data.copy()  # battery tons, kept for logistics
evb_type_data_batt = evb_type_data.copy()  # battery tons by type, kept for economic calculations

# Convert battery quantities (tons battery) → lithium metal (tons Li) by type
metal_content_df = pd.read_csv('./cost/Metal content.csv')
li_content_dict = metal_content_df.dropna(subset=['Type']).set_index('Type')['Li'].to_dict()
li_recovery_lookup = load_li_recovery_lookup()
evb_type_data['scrap'] = evb_type_data.apply(
    lambda row: row['scrap'] * li_content_dict.get(row['type'], 0), axis=1
)
evb_type_data['inuse'] = evb_type_data.apply(
    lambda row: row['inuse'] * li_content_dict.get(row['type'], 0), axis=1
)
# Rebuild total from Li-converted type data
evb_data = evb_type_data.groupby(['Year', 'region'])[['scrap', 'inuse']].sum().reset_index()
evb_data = evb_data.rename(columns={'inuse': 'inuse_sum'})

year_val = 2025
year_val1 = 2050
year_val = int(os.environ.get('SCENARIO_YEAR_START', year_val))
year_val1 = int(os.environ.get('SCENARIO_YEAR_END', year_val1))
scenario_years = os.environ.get('SCENARIO_YEARS')
selected_years = [int(y.strip()) for y in scenario_years.split(',') if y.strip()] if scenario_years else None
#
...
nation_df_copy = nation_df_copy.copy()
...
combined_data_new = combined_data_new.reset_index(drop=True)
...
grouped_cost_coun_df = producer_country_df.groupby(['Recycling_capacity', 'Sub-region Name']).mean(numeric_only=True).reset_index()
grouped_cost_coun_df1 = producer_country_df.groupby(['Recycling_capacity', 'continent']).mean(numeric_only=True).reset_index()
grouped_cost_coun_df2 = producer_country_df.groupby(['Recycling_capacity']).mean(numeric_only=True).reset_index()
...
ev_battery_selected = evb_data_batt[(evb_data_batt['Year'] <= 2050) & (evb_data_batt['region'].isin(producer_list))][
    ['Year', 'region', 'scrap']]
...
for year in list(range(year_val, year_val1 + 1)):
...
pivot_df = ev_battery_selected.pivot(index='Year', columns='region', values='scrap')
...
recycling_cap_data = pd.read_csv('recycling_cap_2050.csv')
...
melt_recycling_cap_df = melt_recycling_cap_df.copy()
...
# val_list = [2033]
val_list = selected_years or list(range(year_val, year_val1 + 1))
for year_val in val_list:
    directory = f'{trans_paths_root}/{year_val}'
    ...
    # China does not export waste batteries: Basel Convention Annex VIII (A1180) classifies
    # spent LiBs as hazardous waste; Chinese MEE regulations prohibit cross-border transfer.
    # Domestic recycling capacity is sufficient (Gaines et al. 2021, Joule). Force self-disposal.
    pivot_diff_chain_df = pivot_diff_chain_df[pivot_diff_chain_df['region'] != 'China']
...
for year_val in val_list:
    for Strategy_type in ['Strategy 2', 'Strategy 3']:
        os.makedirs(result_path(year_val), exist_ok=True)
        path_file = scenario_path(year_val, f'{Strategy_type}_{recycl_proce}_optimal_transportation_plan.csv')
        ...
        if 'Strategy 3' in path_file:
            pro_tran_vol = pd.read_csv(scenario_path(year_val, 'transport_log.csv'))
        ...
        evb_type_data_batt[(evb_type_data_batt['Year'] == year_val) & (~evb_type_data_batt['type'].isin(['TLB']))][
            ['region', 'type', 'scrap']].copy()
        prod_scrap_type = ev_battery_type_selected.copy()
        total_export_weight = prod_scrap_type.groupby('region')['scrap'].sum().rename('Totalscrap')
        ...
        prod_scrap_type['weights'] = (prod_scrap_type['scrap'] / prod_scrap_type['Totalscrap'])
        ...
        merged_scrap_df['scrap'] = merged_scrap_df['self_dispos_count'] + merged_scrap_df['scrap'].fillna(0)
        merged_scrap_df = merged_scrap_df[['iso3', 'type', 'scrap']]
        ...
        ev_battery_type_selected_chios = ev_battery_type_selected_chios.groupby(['country', 'type'])[
                                             'scrap'].max().reset_index().iloc[:, :2]
        disposal_mass_diff_stra_df = pd.DataFrame()

        net_profit_strat1_chiose2, net_profit_strat = fun_class.OUTPUTNetProfitofOptimalTwoThree(
        ...
        net_profit_strat1_chiose2_merge = ev_battery_type_selected_chios[colums[:2]].merge(
            net_profit_strat1_chiose2.iloc[:, [0, 1, 2, 4]],
            on=['country', 'type'],
            how='left')
        numeric_cols = net_profit_strat1_chiose2_merge.select_dtypes(include=[np.number]).columns
        text_cols = [col for col in net_profit_strat1_chiose2_merge.columns if col not in numeric_cols]
        net_profit_strat1_chiose2_merge[numeric_cols] = net_profit_strat1_chiose2_merge[numeric_cols].fillna(0)
        net_profit_strat1_chiose2_merge[text_cols] = net_profit_strat1_chiose2_merge[text_cols].fillna('')
        net_profit_strat1_chiose2_merge = add_scenario_columns(net_profit_strat1_chiose2_merge)
        net_profit_strat = add_scenario_columns(net_profit_strat)
        net_profit_strat1_chiose2_merge.to_csv(result_path(year_val, f'net_profit_{recycl_op}_{Strategy_type}.csv'),
                                               index=True)
        net_profit_strat.to_csv(result_path(year_val, f'net_profit_all_{Strategy_type}.csv'), index=True)

        net_profit_strat1_chiose2['total_netprofits'] = net_profit_strat1_chiose2['scrap'] * \
                                                        net_profit_strat1_chiose2['net_profit'] * 1000
        net_profit_strat_country = net_profit_strat1_chiose2.groupby('country')[
            ['total_netprofits', 'scrap']].sum().reset_index()
        net_profit_strat_country['net_profit'] = net_profit_strat_country['total_netprofits'] / \
                                                 net_profit_strat_country['scrap'] / 1000

        net_profit_strat['unit_cost'] = net_profit_strat['unit_cost'].astype(float)
        idx = net_profit_strat.iloc[:, [0, 1, 2, 4]].groupby(['country', 'type'])['unit_cost'].idxmin()

        net_profit_strat_chiose = net_profit_strat.loc[
            idx, ['country', 'type', 'recycling_m', 'scrap', 'unit_cost']]

        net_profit_strat_chiose['total_costs'] = net_profit_strat_chiose['scrap'] * net_profit_strat_chiose[
            'unit_cost'] * 1000

        cost_strat_country = net_profit_strat_chiose.groupby('country')[
            ['total_costs', 'scrap']].sum().reset_index()
        cost_strat_country['unit_cost'] = cost_strat_country['total_costs'] / \
                                          cost_strat_country['scrap'] / 1000
        cost_strat_country = add_scenario_columns(cost_strat_country)
        cost_strat_country.to_csv(result_path(year_val, f'country_cost_{recycl_op}_{Strategy_type}.csv'), index=True)

        disposal_mass_diff_stra_df = disposal_mass_diff_stra_df.append(
            net_profit_strat1_chiose2[['country', 'type', 'recycling_m', 'scrap']])
        ...
        disposal_mass_diff_stra_df = add_scenario_columns(disposal_mass_diff_stra_df)
        disposal_mass_diff_stra_df = add_lithium_metrics(disposal_mass_diff_stra_df, li_content_dict, li_recovery_lookup)

        recycling_emission_df = recycling_emission_count_chiose.rename(columns={'battery_type': 'type'})
        merged_scrap_recycling_emission = disposal_mass_diff_stra_df.merge(recycling_emission_df,
                                                                           on=['type', 'recycling_m'], how='left')
        merged_scrap_recycling_emission['CO2_em'] = merged_scrap_recycling_emission['CO2'] * \
                                                    merged_scrap_recycling_emission['scrap']
        #
        net_profit_strat_country.to_csv(
            result_path(year_val, f'country_net_profit_{recycl_op}_{Strategy_type}.csv'), index=True)
        disposal_mass_diff_stra_df1 = disposal_mass_diff_stra_df1.append(disposal_mass_diff_stra_df)
        CO2_em_disposal_mass_diff_stra = CO2_em_disposal_mass_diff_stra.append(
            merged_scrap_recycling_emission)
    ...
    net_profit_strat1_chiose1, net_profit_strat1 = fun_class.OUTPUTNetProfitofOptimalOne(ev_battery_type_selected,
                                                                                         pivot_diff_df_cho_copy,
                                                                                         Recycling_revenue_df,
                                                                                         carbon_cost, df_cost_fit_data,
                                                                                         dist_cost)
    net_profit_strat1_chiose1 = add_scenario_columns(net_profit_strat1_chiose1)
    net_profit_strat1 = add_scenario_columns(net_profit_strat1)
    net_profit_strat1_chiose1.iloc[:, [0, 1, 2, 4]].to_csv(result_path(year_val, f'net_profit_{recycl_op}_Strategy 1.csv'), index=True)
    net_profit_strat1.to_csv(result_path(year_val, 'net_profit_all_Strategy 1.csv'), index=True)

    net_profit_strat1_chiose1['total_netprofits'] = net_profit_strat1_chiose1['scrap'] * net_profit_strat1_chiose1[
        'net_profit'] * 1000
    net_profit_strat_country = net_profit_strat1_chiose1.groupby('country')[
        ['total_netprofits', 'scrap']].sum().reset_index()
    net_profit_strat_country['net_profit'] = net_profit_strat_country['total_netprofits'] / net_profit_strat_country[
        'scrap'] / 1000

    net_profit_strat1['unit_cost'] = net_profit_strat1['unit_cost'].astype(float)
    idx = net_profit_strat1.iloc[:, [0, 1, 2, 4]].groupby(['country', 'type'])['unit_cost'].idxmin()

    net_profit_strat_chiose = net_profit_strat1.loc[
        idx, ['country', 'type', 'recycling_m', 'scrap', 'unit_cost']]
    ...
    disposal_mass_diff_stra_df = disposal_mass_diff_stra_df.append(
        net_profit_strat1_chiose1[['country', 'type', 'recycling_m', 'scrap']])
    disposal_mass_diff_stra_df['year'] = [year_val] * len(disposal_mass_diff_stra_df)
    ...
    cost_strat_country = add_scenario_columns(cost_strat_country)
    net_profit_strat_country = add_scenario_columns(net_profit_strat_country)
    cost_strat_country.to_csv(result_path(year_val, f'country_cost_{recycl_op}_Strategy 1.csv'), index=True)
    net_profit_strat_country.to_csv(result_path(year_val, f'country_net_profit_{recycl_op}_Strategy 1.csv'), index=True)
    disposal_mass_diff_stra_df1 = disposal_mass_diff_stra_df1.append(disposal_mass_diff_stra_df)
    CO2_em_disposal_mass_diff_stra = CO2_em_disposal_mass_diff_stra.append(merged_scrap_recycling_emission)

# 删除重复值所在行
CO2_em_disposal_mass_diff_stra = CO2_em_disposal_mass_diff_stra.drop_duplicates(subset=['country', 'type', 'recycling_m', 'year', 'Strategy type'], keep='first')
CO2_em_disposal_mass_diff_stra = CO2_em_disposal_mass_diff_stra.reset_index(drop=True)

CO2_em_df = add_scenario_columns(CO2_em_df)
CO2_em_disposal_mass_diff_stra = add_scenario_columns(CO2_em_disposal_mass_diff_stra)
disposal_mass_diff_stra_df1.to_csv(result_path(f'country_disposal_mass_diff_stra_{recycl_op}.csv'), index=True)
CO2_em_df.to_csv(result_path(f'CO2_em_trans_{recycl_op}.csv'), index=True)
CO2_em_disposal_mass_diff_stra.to_csv(result_path(f'CO2_em_disposal_mass_diff_stra_{recycl_op}.csv'), index=True)

...
DATA_FILE = (
    ROOT / "unified_policy_run_v2" / "data" / "lithium_loss_scenarios_unified"
    / "lithium_loss_scenarios_summary.csv"
)
OUT_DIR = ROOT / "unified_policy_run_v2" / "figures" / "fig4_mitigation_marginal"
...
def load_marginal_for_year(year):
    ...
    delta = pivot.subtract(base, axis=1) / 1000.0
    return delta
...
def main():
    ...
    for ax_idx, (year, ax) in enumerate(zip(YEARS, axes)):
        ...
        for i, mit in enumerate(MITIGATION_ORDER):
            ...
            for j, pol in enumerate(POLICY_ORDER):
                ...
                if abs(value) >= max(1, year_xmax * 0.025):
                    ha = "left" if value >= 0 else "right"
                    offset = 5 if value >= 0 else -5
                    ax.annotate(f"{value:+.0f}", xy=(value, y), xytext=(offset, 0),
        ...
        ax.set_xlabel(f"Marginal benefit (kt available Li) — {year}")
    ...
    fig.suptitle(
        "Marginal benefit of mitigation on supply-chain-available Li (2030 / 2040 / 2050)",
    ...
    pd.concat(long_rows, ignore_index=True).to_csv(
        OUT_DIR / "mitigation_marginal_benefit_2030_2040_2050.csv", index=False
    )
    png = OUT_DIR / "mitigation_marginal_benefit_2030_2040_2050.png"
    pdf = OUT_DIR / "mitigation_marginal_benefit_2030_2040_2050.pdf"

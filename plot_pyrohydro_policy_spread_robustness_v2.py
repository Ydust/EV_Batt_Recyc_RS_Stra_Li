...
OUT_DIR = ROOT / "unified_policy_run_v2" / "figures" / "fig3_pyrohydro_robustness"
...
SCENARIOS = [
    {
        ...
        "dir": ROOT / "unified_policy_run_v2" / "data" / "fig3_pyrohydro_sensitivity_unified" / "pyrohydro_sensitivity_conservative_unified",
        "pyro_weight": 0.35,
        "developed": 0.85,
        "ev_producer": 0.90,
        "other": 1.00,
    ...
    {
        "scenario": "S2 low-mid",
        "dir": ROOT / "unified_policy_run_v2" / "data" / "fig3_pyrohydro_sensitivity_unified" / "pyrohydro_sensitivity_s2_unified",
        "pyro_weight": 0.30,
        "developed": 0.815,
        "ev_producer": 0.86,
        "other": 0.95,
    ...
    {
        "scenario": "S3 medium",
        "dir": ROOT / "unified_policy_run_v2" / "data" / "fig3_pyrohydro_sensitivity_unified" / "pyrohydro_sensitivity_medium_unified",
        "pyro_weight": 0.25,
        "developed": 0.78,
        "ev_producer": 0.82,
        "other": 0.90,
    },
    {
        ...
        "dir": ROOT / "unified_policy_run_v2" / "data" / "fig3_pyrohydro_sensitivity_unified" / "pyrohydro_sensitivity_s4_unified",
        "pyro_weight": 0.20,
        "developed": 0.74,
        "ev_producer": 0.785,
        "other": 0.875,
    ...
    {
        "scenario": "S5 strong",
        "dir": ROOT / "unified_policy_run_v2" / "data" / "fig3_pyrohydro_sensitivity_unified" / "pyrohydro_sensitivity_s5_unified",
        "pyro_weight": 0.15,
        "developed": 0.70,
        "ev_producer": 0.75,
        "other": 0.85,
...
def _plot_radar_combined(ax, share_delta, policy, limit):
    ...
    label_theta = 7 * np.pi / 5  # midway between Africa (6π/5) and Oceania (8π/5)
    for r_scaled, pp_value in [(0.0, -limit), (0.5, 0), (1.0, +limit)]:
        sign = "+" if pp_value > 0 else ("−" if pp_value < 0 else "")
        ax.text(label_theta, r_scaled, f"{sign}{abs(int(pp_value))} pp",
                fontsize=7.5, color="0.40", ha="center", va="center",
                zorder=5,
                bbox=dict(boxstyle="round,pad=0.18", facecolor="white",
                          edgecolor="0.75", linewidth=0.4, alpha=0.95))
...
def plot_policy_pathways_with_share_heatmap(pathway_band, share_delta):
    ...
    year_handles = [
        Line2D([0], [0], color="#555555", lw=1.3, linestyle="--", label="Radar 2040"),
...
def plot_scatter_radar_combined(pathway_band, data, share_delta):
    ...
    radar_limit = 16.0
    LOSS_LIM = 400  # zoom for cumulative loss scatter; outliers clipped + annotated

    # Compute cumulative Li loss vs open_policy
    raw_loss = data[data["technology"].isin(TECHNOLOGY_ORDER)].copy()
    open_df = (
        raw_loss[raw_loss["policy_scenario"] == "open_policy"]
        [["scenario", "year", "technology", "recovered_lithium_t"]]
        .rename(columns={"recovered_lithium_t": "open_li_t"})
    )
    other = raw_loss[raw_loss["policy_scenario"] != "open_policy"].copy()
    merged_loss = other.merge(open_df, on=["scenario", "year", "technology"], how="left")
    merged_loss["loss_annual_t"] = merged_loss["recovered_lithium_t"] - merged_loss["open_li_t"]
    merged_loss = merged_loss.sort_values(["scenario", "policy_scenario", "technology", "year"])
    merged_loss["loss_cum_kt"] = merged_loss.groupby(
        ["scenario", "policy_scenario", "technology"]
    )["loss_annual_t"].cumsum() / 1000.0
    ...
    # Build cumulative loss xy pairs per comparison policy
    cur_loss = merged_loss[merged_loss["policy_scenario"] == BASELINE_POLICY][
        ["scenario", "year", "technology", "loss_cum_kt"]
    ].rename(columns={"loss_cum_kt": "x_kt"})
    central_scenario_name = "S3 medium"

    # Reorder scatter panels: Reference, Critical-route, Strict
    SCATTER_ORDER = ["reference_policy", "critical_route_policy", "strict_policy"]
    panel_top_map = dict(zip(SCATTER_ORDER, panel_top))

    for ax, policy in zip(scatter_axes, SCATTER_ORDER):
        panel = panel_top_map[policy]
        pol_loss = merged_loss[merged_loss["policy_scenario"] == policy][
            ["scenario", "year", "technology", "loss_cum_kt"]
        ].rename(columns={"loss_cum_kt": "y_kt"})
        merged_xy = cur_loss.merge(pol_loss, on=["scenario", "year", "technology"], how="inner")
        med = merged_xy[merged_xy["scenario"] == central_scenario_name]
        others = merged_xy[merged_xy["scenario"] != central_scenario_name]

        # Shade |y|<|x| region (policy closer to open than current)
        xx = np.linspace(-LOSS_LIM, LOSS_LIM, 200)
        ax.fill_between(xx, -np.abs(xx), np.abs(xx),
                        color="#88c98c", alpha=0.08, zorder=0)
        for r, c_ in [(100, "#a8e6a8"), (300, "#dceedd")]:
            ax.add_patch(plt.Rectangle((-r, -r), 2*r, 2*r, fill=False,
                                        edgecolor=c_, linewidth=1.0, linestyle=":", zorder=0))

        for technology in TECHNOLOGY_ORDER:
            sub_o = others[others["technology"] == technology]
            if not sub_o.empty:
                ax.scatter(sub_o["x_kt"], sub_o["y_kt"],
                           color=TECHNOLOGY_COLORS[technology],
                           alpha=0.22, s=28, edgecolor="none", linewidth=0)
            sub_m = med[med["technology"] == technology].sort_values("year")
            if sub_m.empty:
                continue
            non_key = sub_m[~sub_m["year"].isin(key_years)]
            if not non_key.empty:
                ax.scatter(non_key["x_kt"], non_key["y_kt"],
                           color=TECHNOLOGY_COLORS[technology],
                           s=28, marker="o", alpha=0.45,
                           edgecolor="none", linewidth=0, zorder=3)
            for _, row in sub_m[sub_m["year"].isin(key_years)].iterrows():
                x_clip = max(-LOSS_LIM * 0.95, min(LOSS_LIM * 0.95, row["x_kt"]))
                y_clip = max(-LOSS_LIM * 0.95, min(LOSS_LIM * 0.95, row["y_kt"]))
                outside = (abs(row["x_kt"]) > LOSS_LIM) or (abs(row["y_kt"]) > LOSS_LIM)
                ...
                if outside:
                    ...
                    ax.annotate(f"({row['x_kt']:.0f}, {row['y_kt']:.0f})",
                                xy=(x_clip, y_clip),
                                xytext=(x_off, y_off),
        ...
        ax.plot([-LOSS_LIM, LOSS_LIM], [-LOSS_LIM, LOSS_LIM],
                color="0.55", linewidth=0.9, linestyle="--", zorder=1)
        ax.axhline(0, color="0.35", linewidth=0.7, zorder=1)
        ax.axvline(0, color="0.35", linewidth=0.7, zorder=1)
        ax.set_xlim(-LOSS_LIM, LOSS_LIM)
        ax.set_ylim(-LOSS_LIM, LOSS_LIM)
        ax.set_aspect("equal", adjustable="box")
        ...
        for spine in ["top", "right"]:
            ax.spines[spine].set_visible(False)
    scatter_axes[0].set_ylabel("Policy cumulative Li loss (kt)")

    for ax, policy, panel in zip(radar_axes, SCATTER_ORDER, panel_bot):
        _plot_radar_combined(ax, share_delta, policy, radar_limit)
    ...
    fig.suptitle(
        "Policy effects: cumulative Li loss (top) and continent technology-share delta vs Current (bottom)",
        y=0.965, fontsize=12,
    )
    OUT_DIR.mkdir(parents=True, exist_ok=True)

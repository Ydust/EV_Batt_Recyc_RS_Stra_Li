...
INPUT_FILE = (
    ROOT
    / "unified_policy_run_v2"
    / "data"
    / "fig2"
    / "routes_for_plot.csv"
)
OUTPUT_DIR = ROOT / "unified_policy_run_v2" / "figures" / "fig2_route_maps"
...
def plot(by_policy, by_route):
    ...
    for policy in POLICIES:
        subset = by_policy[by_policy["policy_scenario"] == policy].sort_values("year")
        if subset.empty:
            continue
        ax.plot(
            subset["year"], subset["route_access_loss_kt_li"],
        ...
        # Annotate peak value
        if not subset.empty:
            peak_idx = subset["route_access_loss_kt_li"].idxmax()
            peak_row = subset.loc[peak_idx]
            if peak_row["route_access_loss_kt_li"] >= 100:
                ax.annotate(
                    f"{peak_row['route_access_loss_kt_li']:.0f}",
                    xy=(peak_row["year"], peak_row["route_access_loss_kt_li"]),
